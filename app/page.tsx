    ...
    async function loadData() {
      try {
        ...
        if (apiMarkets.length > 0 && apiSnapshots.length > 0) {
          const dates = apiSnapshots.map((s: any) => new Date(s.survey_date).getTime()).filter((t: number) => !isNaN(t));
          if (dates.length > 0) {
            const maxDate = new Date(Math.max(...dates));
            setLatestDataDate(maxDate.toLocaleDateString('en-PH', { year: 'numeric', month: 'long', day: 'numeric' }));
          }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F8FAFC", fontFamily: "var(--font-sans)", color: "var(--text-primary)" }}>
      ...
      <header style={{ background: "white", borderBottom: "1px solid #E5E7EB", position: "sticky", top: 0, zIndex: 100 }}>
        <style>{`
          .mobile-menu-btn { display: none; background: none; border: none; color: var(--color-accent); cursor: pointer; padding: 4px; }
          .nav-actions { display: flex; gap: var(--space-4); alignItems: center; }
          @media (max-width: 768px) {
            .mobile-menu-btn { display: flex; }
            .nav-actions { display: none; }
            .nav-actions.open {
              display: flex; flex-direction: column; align-items: stretch; position: absolute; top: 100%; left: 0; right: 0; background: white; padding: 16px; border-bottom: 1px solid #E5E7EB; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); z-index: 99;
            }
            .nav-actions.open > * { width: 100%; justify-content: flex-start; }
          }
        `}</style>
        <div style={{ maxWidth: 1400, margin: "0 auto", padding: "12px var(--space-6)", minHeight: 70, display: "flex", alignItems: "center", justifyContent: "space-between", gap: "var(--space-4)" }}>
          ...
          <div className={`nav-actions ${isMobileMenuOpen ? 'open' : ''}`}>
            {latestDataDate && (
              <div style={{ display: "flex", alignItems: "center", gap: "var(--space-2)", background: "#ECFDF5", color: "#047857", padding: "6px 14px", borderRadius: "var(--radius-full)", fontWeight: 700, fontSize: "var(--text-sm)", border: "1px solid #A7F3D0", boxShadow: "0 2px 4px rgba(16, 185, 129, 0.1)" }}>
                <CheckCircle size={16} /> Latest Prices: {latestDataDate}
              </div>
            )}
            ...
            <button onClick={() => { setIsComplaintModalOpen(true); setIsMobileMenuOpen(false); }} style={{ border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: "6px", background: "#DC2626", color: "white", padding: "6px 14px", borderRadius: "var(--radius-full)", fontWeight: 700, fontSize: "var(--text-sm)", textDecoration: "none", transition: "opacity 0.2s" }} onMouseOver={e => e.currentTarget.style.opacity = '0.9'} onMouseOut={e => e.currentTarget.style.opacity = '1'}>
              <AlertCircle size={16} /> File Complaint
            </button>
            <button onClick={() => { setIsLoginModalOpen(true); setIsMobileMenuOpen(false); }} className="btn btn-ghost btn-sm" style={{ justifyContent: "center" }}>Admin Access</button>
          </div>
        </div>
      </header>
      ...
      <main style={{ maxWidth: 1400, margin: "0 auto", padding: "var(--space-8) var(--space-6)" }}>
        ...
        <div id="market-navigator" style={{ position: "relative", width: "100%", height: "80vh", minHeight: "600px", borderRadius: "var(--radius-lg)", border: "1px solid #E2E8F0", background: "#F8FAFC", marginBottom: "var(--space-10)", overflow: "hidden" }}>
          ...
          <div style={{ position: "absolute", top: "24px", left: "24px", zIndex: 10, width: "380px", maxWidth: "calc(100% - 48px)", display: "flex", flexDirection: "column", gap: "12px", maxHeight: "calc(100% - 48px)", pointerEvents: "none" }}>
            ...
            <div style={{ pointerEvents: "auto", background: "white", borderRadius: "var(--radius-full)", padding: "12px 20px", display: "flex", alignItems: "center", gap: "12px", boxShadow: "0 8px 30px rgba(0,0,0,0.12)", border: "1px solid #E2E8F0" }}>
              ...
              />
              {mapSearch && (
                <button onClick={() => setMapSearch("")} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-muted)", display: "flex", padding: 4 }}>
                  <X size={16} />
                </button>
              )}
            </div>
              ...
              {(() => {
                ...
                if (results.length === 0) return (
                  <div style={{ background: "white", padding: "16px", borderRadius: "var(--radius-lg)", boxShadow: "0 8px 30px rgba(0,0,0,0.12)", border: "1px solid #E2E8F0", fontSize: "14px", color: "var(--text-muted)", textAlign: "center", fontWeight: 600 }}>
                    No stalls found matching '{mapSearch}'.
                  </div>
                );
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px 12px" }}>
                    {getLegendEntries("stall_status").map((entry) => (
                      <div key={entry.key} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        <span style={{ fontSize: "12px", fontWeight: 600, color: "var(--text-secondary)", lineHeight: 1.2 }}>{entry.label}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {/* Section 6: What Changed This Week? (Moved to bottom to replace Commodity Movers) */}
        <div>
          <h2 style={{ fontSize: "var(--text-2xl)", fontWeight: 800, color: "var(--color-accent)", margin: "0 0 var(--space-4) 0", display: "flex", alignItems: "center", gap: "var(--space-2)" }}>
            <Activity size={24} color="var(--color-primary)" /> What Changed This Week?
          </h2>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "var(--space-4)" }}>
            {(() => {
      ...
      {isComplaintModalOpen && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: "var(--space-4)" }}>
          <div style={{ background: "white", borderRadius: "var(--radius-lg)", width: "100%", maxWidth: "500px", display: "flex", flexDirection: "column", maxHeight: "90vh" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "var(--space-4) var(--space-6)", borderBottom: "1px solid #E5E7EB" }}>
              <h2 style={{ fontSize: "var(--text-xl)", fontWeight: 800, margin: 0, color: "var(--text-primary)" }}>File a Public Complaint</h2>
              <button onClick={() => setIsComplaintModalOpen(false)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-muted)" }}>
                <X size={24} />
              </button>
            </div>
            ...
            <div style={{ padding: "var(--space-6)", overflowY: "auto" }}>
              {complaintSubmitted ? (
                <div style={{ textAlign: "center", padding: "var(--space-8) 0" }}>
                  <CheckCircle size={48} color="#16A34A" style={{ margin: "0 auto var(--space-4)" }} />
                  <h3 style={{ fontSize: "var(--text-lg)", fontWeight: 700, color: "var(--text-primary)", marginBottom: "var(--space-2)" }}>Complaint Submitted</h3>
                  <p style={{ color: "var(--text-secondary)", marginBottom: "var(--space-6)" }}>Thank you for reporting this to the Lucena Market Authority. We will review your complaint and contact you shortly.</p>
                  <button onClick={() => { setIsComplaintModalOpen(false); setComplaintSubmitted(false); }} className="btn btn-primary">Done</button>
                </div>
              ) : (
